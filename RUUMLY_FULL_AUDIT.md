...
| P | Issue | Detail | Size |
|---|---|---|---|
| ~~P1~~ | ~~Raw HTML ships the default `<head>`~~ | **PARTLY MISDIAGNOSED, then fixed.** Verifying against production revealed a Cloudflare Worker (`workers/social-preview`) already live on `ruumly.eu/*` that fetches the Vercel origin and rewrites the head from API metadata for **every** visitor, not just crawlers. So city hubs, partner pages and listings were *already* getting route-specific titles in production — my audit read the Vercel origin and assumed that was what crawlers received. What the Worker does **not** cover is any route its API lookup cannot resolve (`specific === false`), which it passes through untouched: that is where the homepage title really was still shipping, confirmed live on `/et/terms`. The prerender fixes exactly those, and the two now compose. **FIXED 2026-08-14.** `scripts/prerender-seo.mjs` runs after `vite build` and writes 220 per-route heads (44 routes × 5 languages) — 8 static pages, 5 verticals × 6 curated cities, 6 city hubs. Titles come from the app's own `seoMeta`/`translations` modules loaded through Vite's SSR pipeline, so the crawler's head cannot drift from the visitor's. Each route is written both as `<path>/index.html` and `<path>.html` because static hosts disagree about clean-URL resolution. | L |
